...
export async function getTokens(args?: Args) {
  ...
  if (args.list) {
    return filteredTokens.filter(
      (t) =>
        (t.backend_id && args.list?.includes(t.backend_id)) ||
        (args.base && t.is_base)
    );
  }
    ...
    return filteredTokens.filter((t) => {
      ...
      if (args.name)
        return t.name
          .toLowerCase()
          .trim()
          .includes(args.name.toLowerCase().trim());
      if (args.description)
        return t.description
          .toLowerCase()
          .trim()
          .includes(args.description.toLowerCase().trim());
      return false;
    });
  }
