...
export function useSearchOwnedTokens(args: { search: string }) {
    ...
    const searchQuery = useQuery({
        queryKey: ['search-owned-tokens', args.search, query.isLoading, query?.data?.length],
        queryFn: async () => {
            const data = query.data ?? []
            if (args.search?.trim()?.length == 0) return data;
            return data?.filter((token) => {
                if (token.name.toLocaleLowerCase().includes(args.search?.toLowerCase().trim())) return true;
                if (token.description.toLocaleLowerCase().includes(args.search?.toLowerCase().trim())) return true;
                return false
            })
        }
    })
