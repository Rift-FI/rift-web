...
async function aggregateBalancesUsd(): Promise<number> {
  ...
  const response = await axios.get<Record<string, { usd: number }>>("https://pro-api.coingecko.com/api/v3/simple/price", {
    params: {
      ids: aggregatedIds,
      vs_currencies: "usd"
    },
  ...
  const total_amount_in_usd = chainBalances?.data?.reduce((agg, chain) => {
    const chain_data = allChains?.find((c) => c?.backend_id == chain?.chainName)
...
export default function useChainsBalance() {
  const query = useQuery({ queryKey: ['chains-balances'], queryFn: aggregateBalancesUsd })
