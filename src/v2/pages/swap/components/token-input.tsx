...
export default function TokenInput(props: Props){
    ...
    const TOKEN = position == "from" ? state.watch("from_token") : state.watch("to_token")
    const CHAIN = position == "from" ? state.watch("from_chain") : state.watch("to_chain") 
    const AMOUNT = position == "from" ? state.watch("amount_in") : state.watch("amount_out")
    ...
    parseFloat(AMOUNT ?? "0") <= (balanceQuery?.data?.amount ?? parseFloat( "0")) : true
    ...
    return (
        <div className="w-full flex flex-col px-5 py-5 rounded-lg bg-surface-subtle gap-2" >
                            ...
                            return (
                                <div className="px-2 py-1 rounded-full bg-surface-alt flex flex-row items-center gap-x-2 cursor-pointer" >
                                    <div className="w-6 h-6 relative" >
                                        ...
                                        />
                                        {
                                            (chainQuery?.data && !tokenDetailsQuery?.data?.is_native) && <img
            ...
            <div className="w-full flex flex-row items-center justify-between text-muted-foreground text-sm font-semibold" >
                ...
                {
                    balanceQuery?.isLoading ? <div/> : <p>{balanceQuery?.data?.amount} {tokenDetailsQuery?.data?.name}</p>
                }
            </div>
        </div>
    )
