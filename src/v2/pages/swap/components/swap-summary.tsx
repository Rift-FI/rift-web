...
export default function SwapSummary(){
    ...
    const INSUFFICIENT_FUNDS = useMemo(()=> {
        ...
        if((fromBalanceQuery.data?.amount ?? 0) && (parseFloat(swapState.amount_in) == 0)) return false;
        return (fromBalanceQuery.data?.amount ?? 0) < parseFloat(swapState.amount_in)
    }, [swapState.amount_in, swapState.from_token, swapState.from_chain, fromBalanceQuery.isLoading])
        ...
        <div className="w-full flex flex-col gap-y-2 flex-1 items-center justify-between" >
            ...
            <div className="flex flex-col w-full px-3 py-2 rounded-lg bg-surface-subtle" >
                <div className="flex flex-row items-center justify-between w-full px-2 py-2 border-b border-accent" >
                    ...
                    {(fromTokenDetailsQuery.isLoading || toTokenDetailsQuery.isLoading || pricingQuery.isLoading) ? <LoadingPill/> :<p className="text-muted-foreground font-semibold" >
                        1 {fromTokenDetailsQuery?.data?.name} &asymp; {pricingQuery?.data?.price ?? 0} {toTokenDetailsQuery?.data?.name}
                    </p>}
                </div>
                ...
                <div className="flex flex-row items-center justify-between w-full px-2 py-2 border-b border-accent" >
                 ...
                 <div className="flex flex-row items-center justify-between w-full px-2 py-2 border-b border-accent" >
                    ...
                    {
                        feeQuery?.isLoading ? <LoadingPill/> : 
                        <p className="font-semibold text-muted-foreground" >
            ...
            <div className="w-full flex flex-row items-center justify-between" >
                    <ActionButton disabled={ INSUFFICIENT_FUNDS || swapState.amount_in == "0" || swapState.amount_in == ""} onClick={handleSwap} loading={swapMutation.isPending} variant={INSUFFICIENT_FUNDS ? "danger" : "secondary"} >
                        {
...
function LoadingPill(){
    return (
        <div className="flex flex-row items-center w-[100px] h-10 rounded-full bg-surface-alt animate-ping" >
