import useOwnedTokens, { useSearchOwnedTokens } from "@/hooks/data/use-owned-tokens";
    ...
    return (
        <div className="w-full flex flex-col  h-full overflow-y-scroll px-5 py-5 relative " >
            ...
            <div className="w-full h-full flex flex-col gap-2" >
                {
                    tokensQuery?.isLoading ? <div className="w-full h-full flex flex-col gap-2" >
                        <div className="rounded-md bg-surface-alt w-full h-[80px] animate-pulse" >

                        </div>
                    </div> : 
                    tokensQuery?.data?.filter((token)=>{
                        if (token.id == to_token && token.chain_id == to_chain) return false;
                        if (search.trim().length == 0) return true;
                        if(token.name.toLowerCase()?.includes(search.toLowerCase().trim())) return true;
                        if(token.description.toLowerCase()?.includes(search.toLowerCase().trim())) return true;
                        return false
                    }).map((token, i)=>{
                        return (
                            <RenderToken onPress={handleTokenSelect} token={token} key={i} />
                        )
                    })
                }
                {
